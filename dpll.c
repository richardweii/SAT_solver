...
int check_sat(CNF cnf)
{
    ...
    {
        if(cnf->clause_set[i]->flag == -1)
            return -1;
        else if(cnf->clause_set[i]->flag == 0)
            return 0;
    }
    return 1;
}
...
int update_cnf(Variable v, CNF cnf)
{
    ...
    while (cr != NULL)
    {
        ...
        else if(cnf->clause_set[cr->order]->length == 0 && L->sign != value)
            return 0;
        L->flag = 0;
        cr = cr->next;
    }
...
int main(int argc, char const *argv[])
{
    ...
    //     exit(0);
    // }
    CNF cnf = input_parse("D:\\WorkSpace\\SAT\\example\\mysample.cnf");
    formula_display(cnf);
    Result res = DPLL(cnf);
    printf("Result:%d time:%d", res.sat, res.time);
    return 0;
}
#endif
